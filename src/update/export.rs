                ...
                async move {
                    ...
                    match file {
                        Some(f) => {
                            ...
                            if let Err(e) = cmd_tx
                                .send(PdfCommand::ExtractText(doc_id, page, resp_tx))
                                .await
                            {
                ...
                async move {
                    ...
                    match file {
                        Some(f) => {
                            ...
                            let (resp_tx, resp_rx) = tokio::sync::oneshot::channel();
                            if let Err(e) = cmd_tx
                                .send(PdfCommand::ExportImage(doc_id, page, zoom, resp_tx))
                                .await
                            {
            ...
            Task::perform(
                async move {
                    ...
                    if let Some(f) = folder {
                        ...
                        let (tx, rx) = tokio::sync::oneshot::channel();
                        let _ = cmd_tx
                            .send(PdfCommand::Split(path, pages, out_dir, tx))
                            .await;
                        match rx.await {
                            Ok(res) => res,
                            Err(_) => Err(crate::models::PdfError::EngineDied),
        ...
        Message::PrintWithPrinter(payload) => {
            ...
            Task::perform(
                async move {
                    ...
                    let _ = cmd_tx
                        .send(PdfCommand::PrintPdf(path, printer_opt, tx))
                        .await;
                    match rx.await {
                        Ok(res) => res,
                        Err(_) => Err(crate::models::PdfError::EngineDied),
                ...
                async move {
                    ...
                    match save {
                        Some(f) => {
                            ...
                            let (tx, rx) = tokio::sync::oneshot::channel();
                            let _ = cmd_tx
                                .send(PdfCommand::AddWatermark(path, text, out.clone(), tx))
                                .await;
                            match rx.await {
                                Ok(Ok(path)) => Ok(path),
                                Ok(Err(e)) => Err(e),
